...
build_all()
{
    ...
    if [[ ! ($CI == true) ]]; then
        ...
        build_from_github_preset libbitcoin-network "builds/cmake" ${REPO_PRESET[libbitcoin-network]} "$PARALLEL" true "yes" "${BITCOIN_NETWORK_OPTIONS[@]}" $CUMULATIVE_FILTERED_ARGS_CMAKE "$@"
    else
