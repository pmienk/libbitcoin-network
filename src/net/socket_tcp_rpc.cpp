...
// TODO: this is identical to WS-RPC except for selections of the variant
...
void socket::tcp_rpc_read(http::flat_buffer& buffer, rpc::request& request,
    count_handler&& handler) NOEXCEPT
{
    ...
    boost::asio::dispatch(strand_,
        std::bind(&socket::do_tcp_rpc_read,
            shared_from_this(), ec, zero, in, std::move(handler)));
}

// private
void socket::do_tcp_rpc_read(boost_code ec, size_t total,
    const read_rpc::ptr& in, const count_handler& handler) NOEXCEPT
{
    ...
    VARIANT_DISPATCH_METHOD(get_tcp(),
        async_read_some(in->buffer.prepare(size),
            std::bind(&socket::handle_tcp_rpc_read,
                shared_from_this(), _1, _2, total, in, handler)));
}

// private
void socket::handle_tcp_rpc_read(boost_code ec, size_t size, size_t total,
    const read_rpc::ptr& in, const count_handler& handler) NOEXCEPT
{
    BC_ASSERT(stranded());
    ...
    do_tcp_rpc_read(ec, total, in, handler);
}

/// TCP-RPC (write).
// ----------------------------------------------------------------------------

void socket::tcp_rpc_write(rpc::response& response,
    count_handler&& handler) NOEXCEPT
{
    boost_code ec{};
    ...
    boost::asio::dispatch(strand_,
        std::bind(&socket::do_tcp_rpc_write,
            shared_from_this(), ec, zero, out, std::move(handler)));
}

// private
void socket::do_tcp_rpc_write(boost_code ec, size_t total,
    const write_rpc::ptr& out, const count_handler& handler) NOEXCEPT
{
    ...
    if (ec)
    {
        ...
        const auto code = error::rpc_to_error_code(ec);
        if (code == error::unknown) logx("tcp_rpc-write", ec);
        handler(code, total);
        return;
    }
    ...
    VARIANT_DISPATCH_FUNCTION(boost::asio::async_write, get_tcp(),
        buffer.value().first,
            std::bind(&socket::handle_tcp_rpc_write,
                shared_from_this(), _1, _2, total, out, handler));
}

// private
void socket::handle_tcp_rpc_write(boost_code ec, size_t size, size_t total,
    const write_rpc::ptr& out, const count_handler& handler) NOEXCEPT
{
    BC_ASSERT(stranded());
    ...
    // Handle error condition or incomplete message.
    do_tcp_rpc_write(ec, total, out, handler);
}
...
void socket::tcp_rpc_notify(rpc::request& notification,
    count_handler&& handler) NOEXCEPT
{
    boost_code ec{};
    ...
    // Dispatch success or fail, for handler invoke on strand.
    boost::asio::dispatch(strand_,
        std::bind(&socket::do_tcp_rpc_notify,
            shared_from_this(), ec, zero, out, std::move(handler)));
}

// private
void socket::do_tcp_rpc_notify(boost_code ec, size_t total,
    const notify_rpc::ptr& out, const count_handler& handler) NOEXCEPT
{
    ...
    if (ec)
    {
        // Json serializer emits rpc, http and json codes.
        const auto code = error::rpc_to_error_code(ec);
        if (code == error::unknown) logx("tcp-rpc-notify", ec);
        handler(code, total);
        return;
    }
    ...
    VARIANT_DISPATCH_FUNCTION(boost::asio::async_write, get_tcp(),
        buffer.value().first,
            std::bind(&socket::handle_tcp_rpc_notify,
                shared_from_this(), _1, _2, total, out, handler));
}

// private
void socket::handle_tcp_rpc_notify(boost_code ec, size_t size, size_t total,
    const notify_rpc::ptr& out, const count_handler& handler) NOEXCEPT
{
    BC_ASSERT(stranded());
    ...
    // Handle error condition or incomplete message.
    do_tcp_rpc_notify(ec, total, out, handler);
}
